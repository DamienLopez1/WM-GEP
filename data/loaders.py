...
class RolloutSequenceDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    ...
    def _get_data(self, data, seq_index):
        ...
        obs_data = obs_data.flatten().astype(np.float32)
        obs_data = obs_data.reshape(self._seq_len+1,147)
...
class RolloutObservationDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    ...
    def _get_data(self, data, seq_index):
        return torch.tensor(data['observations'][seq_index].flatten(),dtype = torch.float32)
        #return self._transform(data['observations'][seq_index])
    
class RolloutHiddenDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    """ Encapsulates rollouts.
    ...
    each containing a dictionary with the keys:
        ...
        - rewards: (rollout_len,)
        - terminals: (rollout_len,), boolean
        - hiddens: (rollout_len, *hidden_state_len)
    ...
    def _get_data(self, data, seq_index):
        ...
        return torch.tensor(data['hiddens'][seq_index],dtype = torch.float32)
